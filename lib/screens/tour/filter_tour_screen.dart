...
class FilterTourScreen extends StatefulWidget {
  const FilterTourScreen({
    super.key,
  });

  @override
  // ignore: library_private_types_in_public_api
  _FilterTourScreenState createState() => _FilterTourScreenState();
}

class _FilterTourScreenState extends State<FilterTourScreen> {
  DateTime? startDate;
  DateTime? endDate;
  ...
  final List<String> tourServices = [
    "Cultural".tr,
    "Nature & Adventure".tr,
    "Marine".tr,
    "Independent".tr,
  ...
  void _applyFilters() async {
    ...
    final tourTypesStr =
        selectedTourTypes.join(',').toLowerCase().replaceAll(' ', '-');
    final facilitiesStr =
    ...
    if (facilitiesStr.isNotEmpty) attrs['1'] = facilitiesStr;
    if (servicesStr.isNotEmpty) attrs['2'] = servicesStr;
    ...
    await tourProvider.tourlistapi(2, searchParams: {
      ...
      'tour_types': tourTypesStr,
      'facilities': facilitiesStr,
      'travel_styles': servicesStr,
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            _buildSectionTitle('Tour Type'.tr),
            const SizedBox(
              height: 10,
            ),
            FilterChipWidget(
              items: tourTypes,
              selectedItems: selectedTourTypes,
              onSelectionChanged: (selected) {
                setState(() {
                  selectedTourTypes = selected;
                });
              },
            ),
            const Divider(thickness: 1),
            ...
            _buildSectionTitle('Travel Styles'.tr),
            const SizedBox(
              height: 10,
            ),
            FilterChipWidget(
