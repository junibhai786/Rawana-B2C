...
import 'package:moonbnd/screens/tour/tour_confirm_screen.dart';
...
class _BookingScreenState extends State<TourBookingScreen> {
  DateTime? startDate;
  DateTime? endDate;
  ...
  Future<void> _fetchCountries() async {
    ...
    await homeProvider.fetchCountries();
    // Fetch countries
        ...
        if (homeProvider.paymentMethod == "online") {
          ...
          if (urlGo.contains("https://travolyo.com/api/booking/confirm")) {
            await Navigator.push(
              ...
              MaterialPageRoute(
                builder: (context) =>
                    TourConfirmScreen(bookingCode: widget.bookingCode),
              ),
            );
          }
        } else {
  ...
  Widget build(BuildContext context) {
    ...
    final tourbookingitem = Provider.of<HomeProvider>(context, listen: true);

    log('booking ${tourbookingitem.bookingResponse?.data?.booking?.extraPrice ?? ""}');
    log('persontype ${tourbookingitem.bookingResponse?.data?.booking?.personTypes ?? ""}');
    // log("${tourbookingitem.countries} countryes");

    if (tourbookingitem.bookingResponse == null) {
      return Center(
          ...
          children: [
            ...
            Row(
              children: [
                ...
                //           //     // Review Text
                //           //     Text(
                //           //       '${tourbookingitem.bookingResponse?.data?.booking?.service?.reviewScore ?? ""} (20 Reviews)'
                //           //           .tr,
                //           //       style: TextStyle(
                //           //         fontSize: 14,
                //           //         fontFamily: 'Inter'.tr,
            ...
            Column(
              children: [
                ...(tourbookingitem
                            .bookingResponse?.data?.booking?.personTypes ??
                        [])
                    .map((personType) {
                  return Container(
            ...
            ...(tourbookingitem.bookingResponse?.data?.booking?.buyerFees)!
                .map((element) {
              return Container(
            ...
            ...(tourbookingitem.bookingResponse?.data?.booking?.extraPrice)!
                .map((element) {
              return Container(
                padding: EdgeInsets.only(top: 10),
                child: Row(
                ...
                children: [
                  ...
                  Container(
                    ...
                    child: RadioListTile(
                      ...
                      onChanged: (value) {
                        tourbookingitem.paymentMethod = value!;
                        setState(() {});
                      },
                    ),
                  ),
