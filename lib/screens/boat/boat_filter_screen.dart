...
class FilterBoatScreen extends StatefulWidget {
  const FilterBoatScreen({
    super.key,
  });
...
class _FilterScreenState extends State<FilterBoatScreen> {
  double _minPrice = 50;
  ...
  final List<String> boatAmeneties = [
    "events and meetings".tr,
    "scuba gear".tr,
    "hot tubjacuzzi on deck".tr,
    "gulet".tr,
    "sport fishing".tr,
    "speciality classic yacht".tr,
  ];
  ...
  void initState() {
    ...
    super.initState();
    Provider.of<BoatProvider>(context, listen: false).searchController =
  ...
  void _applyFilters() async {
    ...
    final facilitiesStr =
        selectedAmeneties.join(',').toLowerCase().replaceAll(' ', '-');
    ...
    if (boatTypesStr.isNotEmpty) attrs['14'] = boatTypesStr;
    if (facilitiesStr.isNotEmpty) attrs['15'] = facilitiesStr;

    final provider = Provider.of<BoatProvider>(context, listen: false);

    // Update search parameters and navigate back
    provider.setSelectedHomeTab(7); // Set boat tab as active

    // Use the existing hotellistapi with search parameters
    bool check = await provider.boatlistapi(7, searchParams: {
      ...
      'startDate': startDate,
      'boat_type': boatTypesStr,
      'boat_features': facilitiesStr,
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            _buildSectionTitle('Boat Type'.tr),
            const SizedBox(
              height: 10,
            ),
            FilterChipWidget(
            ...
            _buildSectionTitle('Ameneties'.tr),
            const SizedBox(
              height: 10,
            ),
            FilterChipWidget(
              items: boatAmeneties,
              selectedItems: selectedAmeneties,
              onSelectionChanged: (selected) {
                setState(() {
                  selectedAmeneties = selected;
                });
              },
            ),
  ...
  Widget _buildReviewScoreSelection() {
    return Wrap(
      ...
      children: List.generate(5, (index) {
        ...
        final bool isSelected = selectedReviewScore == index + 1;
