...
class ManageEditCouponScreen extends StatefulWidget {
  CouponManage? booking;
  ManageEditCouponScreen(this.booking, {super.key});
  @override
  State<ManageEditCouponScreen> createState() => _ManageAddCouponScreenState();
}

class _ManageAddCouponScreenState extends State<ManageEditCouponScreen> {
  ...
  void initState() {
    ...
    Provider.of<HomeProvider>(context, listen: false)
        .fetchVendorServices()
        .then((value) {
      unquieCodeController = TextEditingController(text: widget.booking?.code);
      unquieNameController = TextEditingController(text: widget.booking?.name);
      unquieAmountController =
          TextEditingController(text: "${widget.booking?.amount}");
      discountType = widget.booking?.discountType ?? "";
      selectEndDate = DateTime.parse(widget.booking?.endDate ?? "");
      endDateController = TextEditingController(
          text: DateFormat('d/MM/yyyy')
              .format(DateTime.parse(widget.booking?.endDate ?? "")));

      minimumSpendController =
          TextEditingController(text: "${widget.booking?.minTotal}");
      maximumSpendController =
          TextEditingController(text: "${widget.booking?.maxTotal}");

      manageServices = widget.booking?.services[0] ?? "";
      unlimitedUsageCouponController =
          TextEditingController(text: "${widget.booking?.quantityLimit}");
      unlimitedUsageUserController =
          TextEditingController(text: "${widget.booking?.limitPerUser}");

      contentStatus = widget.booking?.status ?? "";
      inputImage = widget.booking?.couponImageUrl ?? "";
                    ...
                    children: [
                      ...
                      featuredImage != null
                          ? Image.file(
                              ...
                              fit: BoxFit.fill)
                          : Image.network(inputImage,
                              height: 140, width: 250, fit: BoxFit.fill),
                      ElevatedButton.icon(
                        onPressed: () {
                          pickImage();
                        },
                        label: Text("Change Image".tr),
                        ...
                        child: ElevatedButton(
                          onPressed: () async {
                            ...
                            if (_formKey.currentState!.validate()) {
                              ...
                              bool check = await itemProvider.editVendorCoupon(
