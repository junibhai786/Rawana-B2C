...
class MapFilterScreen extends StatefulWidget {
  const MapFilterScreen({super.key, required List<HotelList> locations});
...
class _MapFilterScreenState extends State<MapFilterScreen> {
  ...
  void _applyFilters() async {
    // Convert selected types to URL-friendly format
    ...
    if (startDate != null && endDate != null) {
      searchParams['check_in'] = DateFormat('yyyy-MM-dd').format(startDate!);
      searchParams['check_out'] = DateFormat('yyyy-MM-dd').format(endDate!);
    }

    final provider = Provider.of<HomeProvider>(context, listen: false);
    provider.setSelectedHomeTab(1); // Set map tab as active
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            _buildSectionTitle('All Category'.tr),
            const SizedBox(
              height: 10,
            ),
            FilterChipWidget(
                    ...
                    child: Container(
                      ...
                      child: Row(
                        children: [
                          ...
                          SizedBox(width: 12),
                          Text(
                            "${startDate != null ? DateFormat("MMM dd").format(startDate!) : ''} ${endDate != null ? DateFormat("MMM dd").format(endDate!) : 'Check in - Check out'.tr}",
                            style: TextStyle(
                              fontFamily: 'Inter'.tr,
                              color: kPrimaryColor,
                              fontSize: 14,
