...
import 'package:moonbnd/vendor/tour/edittour/edit_tour_pricing_screen.dart';
...
class EditTourLocationScreen extends StatefulWidget {
  String title = "";
  String content = "";
  String tourId = "";
...
class _SpaceLocationScreenState extends State<EditTourLocationScreen> {
  ...
  void update() {
    ...
    locationid = "${spaceProvider.tourlistdetails?.data?[0].location?.id}";
    addresscontroller = TextEditingController(
        text: spaceProvider.tourlistdetails?.data?[0].address);
    latitude = TextEditingController(
        text: "${spaceProvider.tourlistdetails?.data?[0].mapLat}");
    longitude = TextEditingController(
        text: "${spaceProvider.tourlistdetails?.data?[0].mapLng}");

    zoom = TextEditingController(
        text: "${spaceProvider.tourlistdetails?.data?[0].mapZoom}");

    _updateMarker(LatLng(
        double.parse(spaceProvider.tourlistdetails?.data?[0].mapLat ?? ""),
        double.parse(spaceProvider.tourlistdetails?.data?[0].mapLng ?? "")));
    spaceProvider.tourlistdetails?.data?[0].surrounding?["1"]
        ?.forEach((element) {
      transportationList.add(EducationClass(
    ...
    spaceProvider.tourlistdetails?.data?[0].surrounding?["2"]
        ?.forEach((element) {
      healthList.add(EducationClass(
        name: TextEditingController(text: element.name),
        content: TextEditingController(text: element.content),
    ...
    spaceProvider.tourlistdetails?.data?[0].surrounding?["3"]
        ?.forEach((element) {
      educationList.add(EducationClass(
        name: TextEditingController(text: element.name),
        content: TextEditingController(text: element.content),
  ...
  Widget build(BuildContext context) {
    ...
    // final spaceProvider =
    //     Provider.of<VendorTourProvider>(context, listen: true);
    return Scaffold(
      appBar: AppBar(
        leading: IconButton(
      ...
      body: Form(
        ...
        child: ListView(
          ...
          children: [
            ...
            // Zoom Level Input
            CustomTextField(hintText: "Map Zoom".tr, controller: zoom),
            SizedBox(height: 20),
            Divider(
              thickness: 2,
            ),
            ...
            TertiaryButton(
              press: () async {
                print("Location Screen - Navigation Data:");
                print("Tour ID: ${widget.tourId}");
                print("Location ID: $locationid");
                print("Address: ${addresscontroller.text}");
                print("Coordinates: ${latitude.text}, ${longitude.text}");
                print("Education List: ${educationList.length} items");
                print("Health List: ${healthList.length} items");
                print(
                    "Transportation List: ${transportationList.length} items");
                ...
                if (latitude.text.isNotEmpty &&
                    ...
                    zoom.text.isNotEmpty &&
                    addresscontroller.text.isNotEmpty) {
                  Navigator.push(
                    context,
                    MaterialPageRoute(
                        builder: (context) => EditTourPricingScreen(
