...
class BoatFourScreen extends StatefulWidget {
  @override
  BoatFourScreenState createState() => BoatFourScreenState();
}

class BoatFourScreenState extends State<BoatFourScreen> {
                  ...
                  children: <Widget>[
                                        ...
                                        children: [
                                          Checkbox(
                                            value: provider.selectedboattypeIds
                                                .contains(provider.boattypes
                                                    ?.data?[index].id),
                                            onChanged: (value) {
                                              setState(() {
                                                if (value == true) {
                                                  provider.selectedboattypeIds
                                                      .add(provider
                                                              .boattypes
                                                              ?.data?[index]
                                                              .id ??
                                    ...
                                    children: [
                                      Row(
                                        children: [
                                          Checkbox(
                                            value: provider
                                                .selectedboatamenityIds
                                                .contains(provider.boatamenities
                                                    ?.data?[index].id),
                                            onChanged: (value) {
                                              setState(() {
                                                if (value == true) {
                    ...
                    Center(
                      child: TertiaryButton(
                        press: () async {
                          final provider = Provider.of<VendorBoatProvider>(
                              context,
                              listen: false);

                          if (provider.boattypes!.data!.isEmpty) {}
                          ...
                          bool success = await provider.addBoatVendor(
                          ...
                          if (success) {
                            ...
                            provider.youtubeVideoController.clear();
                            provider.bannerImage = null;
                            provider.featuredImage = null;
                            provider.galleryImages.clear();
                            provider.guestController.clear();
                            provider.cabinController.clear();
                            provider.speedController.clear();
                            provider.lengthController.clear();
                            provider.cancelPolicyController.clear();
                            provider.additionalTermsController.clear();

                            provider.minDayBeforeBooking.clear();
                            provider.startTimeController.clear();
                            provider.endTimeController.clear();
                            provider.pricePerHourController.clear();
                            provider.pricePerDayController.clear();
                            provider.locationid = '';
                            provider.addresscontroller.clear();
                            provider.mapLat = '';
                            provider.mapLng = '';
                            provider.mapZoom = '';
                            provider.faqList.clear();
                            provider.specsList.clear();
                            provider.includeList.clear();
                            provider.excludeList.clear();
                            provider.enableExtraPrice = false;
                            provider.extraPriceBoatVendorList.clear();
